					...
					<div class="display-tc">
						<div class="row">
							<div class="col-md-8 animate-box">
								<h1 class="no-margin">Специализации отделений</h1>
								<p>Каждый отделение цпециализируется на определённой отрасли медицины.</p>
							</div>
						</div>
					</div>
				</div>
...
// удаление записи
if($_GET['act'] == 'del'){
	$query = "DELETE FROM specializations WHERE id=".$_POST['id'];
    $result = mysqli_query($connect, $query);
}
// добавление новой записи
if($_GET['todo'] == 'create'){
	$query = "	INSERT INTO specializations(name) 
	            VALUES ('".$_POST['name']."'
	            )";
	$result = mysqli_query($connect, $query);
}
...
if($_GET['act'] == 'edit'){
	...
	<div class="row" style="margin-bottom: 30px;">
	  	<div class="col-md-1"><p style="text-align: right;">Новая специализация: </p></div>
	 	<div class="col-md-1">
